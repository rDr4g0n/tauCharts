...
(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('taucharts'), require('d3-format')) :
	typeof define === 'function' && define.amd ? define(['taucharts', 'd3-format'], factory) :
	(global.Taucharts = global.Taucharts || {}, global.Taucharts.Legend = factory(global.Taucharts,global.d3));
...
var xml = Taucharts.api.utils.xml;
...
var splitRealValuesEvenly = function (values, count, funcType) {
    ...
    var nextStep = function () {
        ...
        var prevStep = step;
        step = steps[si++];
        step.min = Math.max((step.min), ((prevStep && prevStep.closest !== null ? prevStep.closest : result[0]) + halfStep));
    };
    nextStep();
...
var getNumberFormatter = function (start, end) {
    ...
    return function (num) {
        ...
        var trailingDigits = Math.min(((diffExp < 0 ? Math.abs(diffExp) : 0) +
            (numExp < diffExp ? 1 : 0)), 20); // NOTE: `toFixed` accepts values between 0 and 20.
    ...
    return {
        init: function (chart) {
            ...
            if (hasColorScales || hasFillScales || hasSizeScales) {
                ...
                if (hasColorScales) {
                    _delegateEvent(this._container, 'click', RESET_SELECTOR, function (e, currentTarget) {
                        this._toggleLegendItem(currentTarget, 'reset');
                    }.bind(this));
                    _delegateEvent(this._container, 'click', COLOR_ITEM_SELECTOR, function (e, currentTarget) {
                        var mode = (e.ctrlKey || e.target.matches(COLOR_TOGGLE_SELECTOR) ?
                            'leave-others' :
                            'focus-single');
                        this._toggleLegendItem(currentTarget, mode);
                    }.bind(this));
                    _delegateEvent(this._container, 'mouseover', COLOR_ITEM_SELECTOR, function (e, currentTarget) {
                        this._highlightToggle(currentTarget, true);
                    }.bind(this));
                    _delegateEvent(this._container, 'mouseout', COLOR_ITEM_SELECTOR, function (e, currentTarget) {
        ...
        _drawFillLegend: function () {
            ...
            self._fill.forEach(function (c) {
                ...
                if (firstNode) {
                    ...
                    var gradient = (xml.apply(void 0, ['svg',
                        {
                            class: 'tau-chart__legend__gradient',
                            width: layout.width,
                            height: layout.height
                        },
                        xml('defs', xml.apply(void 0, ['linearGradient',
                            {
                                id: gradientId,
                                x1: '0%',
                        ...
                        xml('rect', {
                            ...
                            fill: 'url(#' + gradientId + ')'
                        })].concat(labels.map(function (text, i) {
                        return xml('text', {
                            x: layout.textX[i],
                            y: layout.textY[i],
            ...
            self._size.forEach(function (c) {
                ...
                if (firstNode) {
                    ...
                    if ((last - first)) {
                        ...
                        var realValues = utils.unique(self._chart
                            .getDataSources({
                    ...
                    var sizeLegend = (xml.apply(void 0, ['svg',
                        {
                            class: 'tau-chart__legend__size',
                            width: layout.width,
                            height: layout.height
                        }].concat(sizes.map(function (size, i) {
                        return xml('circle', {
                            class: ('tau-chart__legend__size__item__circle ' +
        ...
        _toggleLegendItem: function (target, mode) {
            var filters = this._currentFilters;
            var colorNodes = (target ? Array.prototype.filter.call(target.parentNode.childNodes, function (el) {
